...
from scripts.sinkhorn import SinkhornSolver  # Reusing the existing sinkhorn implementation

class ModelTester:
    """
    Class for testing and comparing HMM and RNN models.
    """
    def __init__(self, hmm_generator, rnn_model, num_seq, seq_len, outputs):
        """
        ...
        """
        self.hmm_generator = hmm_generator
    ...
    def generate_test_data(self):
        ...
        self.hmm_data, _ = self.hmm_generator.generate_sequences(self.num_seq, self.seq_len)
        self.hmm_data2, _ = self.hmm_generator.generate_sequences(self.num_seq, self.seq_len)
        self.hmm_data3, _ = self.hmm_generator.generate_sequences(self.num_seq, self.seq_len)
        
        print("Generating RNN test sequences...")
        # Generate RNN sequences
        time_steps = self.num_seq * self.seq_len
        rnn_outputs = self.rnn_model.generate_sequences(time_steps, "full")
    ...
    def match_sequences(self, seq1, seq2):
        """
    ...
    def calculate_euclidean_distances(self):
        ...
        hmm_matched1, hmm_matched2 = self.match_sequences(self.hmm_data2, self.hmm_data3)
        
        # Match HMM with RNN
        hmm_matched_rnn, rnn_matched_hmm = self.match_sequences(self.hmm_data, self.rnn_data)
    ...
    def calculate_volatilities(self, seq):
        """
    ...
    def calculate_frequencies(self, seq):
        """
    ...
    def calculate_transition_matrices(self, seq):
        """
        ...
        for i in range(seq.shape[0]):
            ...
            if np.any(row_sums == 0):
                ...
                for row in zero_rows:
                    mat[row, row] = 1  # Self-transition for unused states
            else:
                mat = mat / row_sums
    ...
    def run_all_tests(self):
        ...
        hmm_volatilities = self.calculate_volatilities(self.hmm_data)
        rnn_volatilities = self.calculate_volatilities(self.rnn_data)
        
        print("Calculating frequencies...")
        hmm_freqs = self.calculate_frequencies(self.hmm_data)
        rnn_freqs = self.calculate_frequencies(self.rnn_data)
        
        print("Calculating transition matrices...")
        hmm_trans = self.calculate_transition_matrices(self.hmm_data)
        rnn_trans = self.calculate_transition_matrices(self.rnn_data)
    ...
    def generate_plots(self, results, save_path=""):
        """
