...
from scripts.plots import get_specific_models
...
def extract_rnn_models():
    ...
    Returns:
        dict: Dictionary of models organized by model_type, hidden_size, input_size, and seed
    """
...
def generate_trajectories_no_input(rnn, pca, num_samples=100, trajectory_length=500):
    ...
    Returns:
        tuple: (trajectories, color_labels, fixed_point)
            ...
            fixed_point: Array of shape (2,) representing the fixed point in PCA space
    """
...
def generate_trajectories_with_input(rnn, pca, num_samples=1, trajectory_length=500, fixed_point_pca=None):
    ...
    Args:
        ...
        num_samples (int): Number of initial conditions to sample
        trajectory_length (int): Length of each trajectory
        fixed_point_pca: Array of shape (2,) representing the fixed point in PCA space
        
    Returns:
        tuple: (trajectories, color_labels, fixed_point_pca)
            trajectories: Array of shape (num_samples, trajectory_length, 2) for 2D PCA projections
            color_labels: Array of shape (num_samples, trajectory_length) for coloring
            fixed_point_pca: Array of shape (2,) representing the fixed point in PCA space
    """
...
def create_grid_plot(model_type, trajectories_dict, color_labels_dict, fixed_points_dict, output_folder, with_input=False):
    ...
    Args:
        model_type (str): Type of the model (e.g., "HMMTwo")
        trajectories_dict (dict): Dictionary containing trajectories for different configurations
        color_labels_dict (dict): Dictionary containing color labels for different configurations
        fixed_points_dict (dict): Dictionary containing fixed points for different configurations
        output_folder (str): Folder to save the plot
        with_input (bool): Whether the trajectories were generated with input
    """
...
def create_variance_contour_grid(model_type, rnn_pca_dict, output_folder):
    ...
    Args:
        model_type (str): Type of the model (e.g., "HMMTwo")
        rnn_pca_dict (dict): Dictionary containing (rnn, pca) tuples for different configurations
        output_folder (str): Folder to save the plot
    """
    # Create figure with 4x3 grid
    fig, axes = plt.subplots(3, 4, figsize=(16, 12))
...
def create_special_variance_contour_plot(model_types, rnn_pca_dict, output_folder):
    ...
    Args:
        ...
        rnn_pca_dict (dict): Dictionary containing (rnn, pca) tuples for different model types
        output_folder (str): Folder to save the plot
    """
...
def create_special_plot(model_types, trajectories_dict, color_labels_dict, fixed_points_dict, output_folder, with_input=False, rnn_pca_dict=None):
    ...
    Args:
        model_types (list): List of model types to include
        trajectories_dict (dict): Dictionary containing trajectories for different model types
        color_labels_dict (dict): Dictionary containing color labels for different model types
        fixed_points_dict (dict): Dictionary containing fixed points for different model types
        output_folder (str): Folder to save the plot
        with_input (bool): Whether the trajectories were generated with input
        rnn_pca_dict (dict, optional): Dictionary containing (rnn, pca) tuples for different model types
    """
    # Create figure with 1x4 grid
    fig, axes = plt.subplots(1, 4, figsize=(20, 5))
    axes = axes.flatten()
...
def run_pipeline(traj_no_in=True, traj_in=True, variance_contour_plots=True):
    ...
    Args:
        traj_no_in (bool): Whether to generate trajectory plots without input
        traj_in (bool): Whether to generate trajectory plots with input
        variance_contour_plots (bool): Whether to generate variance contour plots
    """
