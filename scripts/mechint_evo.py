...
def model_evolution(evolution_dir, best_model_path, num_steps_best=30000, num_steps_other=50, best_steps_to_plot=0):
...
def model_evolution_plotly(evolution_dir, best_model_path, num_steps_best=30000, num_steps_other=50, best_steps_to_plot=0):
    evolution_dir = Path(evolution_dir)
    best_model_path = Path(best_model_path)
    rnn_best = RNN(input_size=100, hidden_size=150, num_layers=1, output_size=3, biased=[False, False])
    rnn_best.load_model(str(best_model_path))
    ...
    for traj, epoch in zip(trajectories_pca, epochs_list):
        ...
        pc2 = np.clip(traj[:, 1], pc2_min, pc2_max)
        z = np.full(pc1.shape, epoch)
        alpha = 0.5 if np.all((traj[:, 0] >= pc1_min) & (traj[:, 0] <= pc1_max) & 
                             (traj[:, 1] >= pc2_min) & (traj[:, 1] <= pc2_max)) else 0.1
        ...
        trace = go.Scatter3d(
            ...
            marker=dict(
                ...
                color=epoch,
                colorscale='Viridis',
                colorbar=dict(title='Epoch', titleside='right')
            ),
            name=f'Epoch {epoch}'
        )
        traces.append(trace)
    ...
    if best_steps_to_plot > 0:
        ...
        best_trace = go.Scatter3d(
            ...
            name='Best Model (first 1k steps)'
        )
    ...
    # Update layout
    fig.update_layout(
        title="3D Trajectories within Best Model's PCA Bounds",
        scene=dict(
            xaxis_title='PC1',
            yaxis_title='PC2',
            zaxis_title='Epoch',
            xaxis=dict(range=[pc1_min, pc1_max]),
            yaxis=dict(range=[pc2_min, pc2_max]),
            zaxis=dict(range=[0, max(epochs_list) * 1.1])
        ),
        width=1200,
        height=1000,
