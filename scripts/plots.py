...
def compare_hmm_rnn_transition_matrices():
    ...
    The function extracts num_seq and seq_len from the model configurations:
    - num_seq: 30,000 for all models
    - seq_len: 
        - 100 for HMMTwo
        - 30 for HMMThree and HMMFour
        - 40 for HMMFive
            ...
            for input_size, seeds in input_sizes.items():
                ...
                for seed, models in seeds.items():
                    ...
                    for model_path, config in models:
                        ...
                        if model_type == "HMMTwo":
                            seq_len = 100
                        elif model_type in ["HMMThree", "HMMFour"]:
                            seq_len = 100
                        elif model_type == "HMMFive":
                            seq_len = 100
                        else:
...
def compare_hmm_rnn_euclidean_distances():
    ...
    This function:
    1. Generates HMM and RNN sequences for each model
    2. Matches them using SinkhornSolver
    3. Calculates Euclidean distances between matched sequences
    4. Averages these distances across seeds
    5. Creates plots showing the distances for different configurations
    ...
    Returns:
        dict: Dictionary containing the averaged Euclidean distances for each model type,
              hidden size, and input size combination
    """
    # Get all models with their configurations
    models_with_configs = get_models_with_configs()
...
if __name__ == "__main__":
    # Uncomment one of the lines below to run the desired comparison
